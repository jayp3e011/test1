	...
	if($link){
		...
		if(isset($_POST['action'])){
			if($_POST['action']=="createquestion"){
				...
				$sql = "insert into $table VALUES('','$topic_id', '$question','$choice_a','$choice_b','$choice_c','$choice_d','$answer','$reference')";
			...
			if($_POST['action']=="updatequestion"){
				...
				$answer = $_POST['answer'];
				$reference = $_POST['reference'];
				$sql = "update $table SET topic_id='$topic_id', question='$question',choice_a='$choice_a',choice_b='$choice_b',choice_c='$choice_c',choice_d='$choice_d',answer='$answer',reference='$reference' where id='$id'";
				$result = mysqli_query($link, $sql) or die("Invalid query" . mysqli_error($link));
				echo "ok";
			}
